...
def visualize_labels(voxel_labels,grid_size,voxel_size):
    ...
    norm_labels = labels / voxel_labels.max()  # 归一化到 [0,1]
...
# def visualize_invalid(invalid,grid_size,voxel_size):
#     # 计算体素的3D坐标
#     indices = np.array(range(voxel_labels.size))  # 线性索引
#     z_indices = indices // (grid_size[0] * grid_size[1])
#     y_indices = (indices % (grid_size[0] * grid_size[1])) // grid_size[0]
...
#     # 只保留非零标签的体素进行可视化（可选）
#     mask = invalid > 0  # 过滤掉 label=0 的背景
#     points = points[mask]
#     labels = invalid[mask]
#     # labels = voxel_labels
...
def main():
    label = check_label("/home/dji/workspace/drone_occ/OccDepth/data/TartanAirOcc/00/voxels/000000.label")
    invalid = check_invalid("/home/dji/workspace/drone_occ/OccDepth/data/TartanAirOcc/00/voxels/000000.invalid")
    occluded = check_occluded("/home/dji/workspace/drone_occ/OccDepth/data/TartanAirOcc/00/voxels/000000.occluded")
    # print(label)
    # print(invalid)
    label[occluded == 1] = 0
    # print(occluded)
